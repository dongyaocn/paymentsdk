...
class Rsa {
    public static function encryptByPrivateKey($data) {
        ...
        $pi_key = openssl_pkey_get_private(file_get_contents($key_path));//这个函数可用来判断私钥是否是可用的，可用返回资源id Resource id
        $encrypted="";
    ...
    public static function decryptByPublicKey($data) {
        ...
        $pu_key = openssl_pkey_get_public(file_get_contents($key_path));//这个函数可用来判断公钥是否是可用的，可用返回资源id Resource id
