...
class TDES {
    ...
    public static function encrypt2HexStr($keys, $sourceData) {
        ...
        //echo ("补位后数据HEX表示:" . Byte::bytesToHex ( $resultByte ) . "<br/>");
        //echo ("秘钥HEX表示:" . Byte::strToHex ( $keys ) . "<br/>");
        //echo ("秘钥长度:" . count ( Byte::getBytes ( $keys ) ) . "<br/>");
        //echo Byte::toStr ( $resultByte );
        $desdata = self::encrypt ( Byte::toStr ( $resultByte ), $keys );
        //echo ("加密后的长度:" . strlen ( $desdata ) . "<br/>");
        return Byte::strToHex ( $desdata );
    }
    ...
    public static function decrypt4HexStr($keys, $data) {
        ...
        $hexSourceData = Byte::hexStrToBytes ($data);
        //var_dump($hexSourceData);

        // 解密
        $unDesResult = self::decrypt (Byte::toStr($hexSourceData),$keys);
        //echo $unDesResult;
        $unDesResultByte = Byte::getBytes($unDesResult);
        ...
        // 有效数据长度
        $dsb = Byte::byteArrayToInt( $dataSizeByte, 0 );
        $tempData = array ();
        for($j = 0; $j < $dsb; $j++) {
            $tempData [$j] = $unDesResultByte [4 + $j];
        }

        return Byte::hexTobin (Byte::bytesToHex ( $tempData ));
