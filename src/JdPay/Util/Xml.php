...
class Xml {
    ...
    public static function encryptReqXml($param){
        ...
        // echo "源串：".htmlspecialchars($xmlStr)."<br/>";exit;
        $sha256SourceSignString = hash("sha256", $xmlStr);
        //echo "摘要:".$sha256SourceSignString."<br/>";
        $sign = Rsa::encryptByPrivateKey($sha256SourceSignString);

        $signDom = $dom->createElement("sign");
        $signDom->nodeValue = $sign;
        $dom->getElementsByTagName('jdpay')->item(0)->appendChild($signDom);

        $data = self::xmlToString($dom);
        // echo "封装后:".htmlspecialchars($data)."<br/>";exit;
        ...
        $encrypt = TDES::encrypt2HexStr($keys, $data);
    ...
    public static function decryptResXml($resultData,&$resData){
        ...
        $desKey = Config::get("desKey");
        $keys = base64_decode($desKey);
        $reqBody = TDES::decrypt4HexStr($keys, $encryptStr);
     ...
     // $bodyDom = self::arrtoxml($bodyObj,0,0);
     ...
     // $reqBodyStr = self::xmlToString($bodyDom);
        ...
        $decryptStr = Rsa::decryptByPublicKey($inputSign);
